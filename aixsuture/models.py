...
from torchvision.models.video import swin3d_t, swin3d_s, swin3d_b, Swin3D_B_Weights
...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        '''
        fit model to case: set heats, dropout ...

        :param num_class: number of classes for head
        '''
        if 'SWINTransformer' in self.arch:
            #self.base_model.dropout = self.dropout

            # replace head layer for 3 classes
            ar = self.arch.split('_')
            key = ar[1]
            if key in ('T', 'S'):
                self.base_model.head = torch.nn.Linear(in_features=768, out_features=num_class)
            elif key == 'B':
                self.base_model.head = torch.nn.Linear(in_features=1024, out_features=num_class)
            else:
                self.base_model.head = torch.nn.Linear(in_features=1024, out_features=num_class)


            self.new_fc = None # don't need fully connected layer bc already covered by replacing head in line above
        elif self.arch == 'Inception3D':
            ...
            if self.new_fc is None:
                # apply a normal distribution with mean 0 and standard deviation std to the weights of the last layer of the base_model
                normal(getattr(self.base_model, self.base_model.last_layer_name).weight, 0, std)
                # set the bias of the last layer of the base_model to 0
    ...
    def _prepare_base_model(self, base_model, pretrained_model=None):
        ...
        :param base_model: base model name gotten from argparse
        :param pretrained_model: pretrained model weights filepath? from argparse
        '''
        if 'SWINTransformer' in base_model:
            if self.modality == 'RGB' or self.use_three_input_channels:
                ar = base_model.split('_')
                key = ar[1]
                if key == 'T':
                    self.base_model = swin3d_t(weights='KINETICS400_V1')
                elif key == 'S':
                    self.base_model = swin3d_s(weights='KINETICS400_V1')
                elif key == 'B':
                    self.base_model = swin3d_b(weights='KINETICS400_V1')
                else:
                    self.base_model = swin3d_b(weights='KINETICS400_V1')

                # mean and stds of image net
                self.input_size = 224
                self.input_mean = [0.485, 0.456, 0.406]
                self.input_std = [0.229, 0.224, 0.225]
        elif base_model == 'Inception3D':
            if self.modality == 'RGB' or self.use_three_input_channels:
    ...
    def forward(self, input):
        ...
        if self.is_3D_architecture:
            # Snippets are merged with batch, so they are predicted independently, will be averaged to one prediction for video later (consensus)
            B, S, C, T, H, W = input.shape
            input = input.view(B * S, C, T, H, W)  # input shape: B*S, C, T, H, W (S = segment, T = snippets length)
        else:
            input = input.view((-1, sample_len) + input.size()[-2:])

        base_out = self.base_model(input)  # base_out shape: B, S, Channels?
        ...
        output = self.consensus(base_out)  # average results of all snippets
    ...
    def is_3D_architecture(self):
        return "3d" in self.arch or "3D" in self.arch or "SWIN" in self.arch
